...
<div class="row justify-content-center">
    <div class="col-md-8 col-lg-6">
        <div class="d-flex align-items-center mb-4">
            ...
            <h3 class="fw-bold text-dark m-0">➕ Tambah Menu Baru</h3>
        </div>
        ...
        <div class="card shadow-sm p-4">
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Kopi Susu Aren" required>
                </div>

                <div class="mb-3">
                    <label for="price" class="form-label small fw-bold text-secondary">Harga Menu (Rp)</label>
                    <input type="number" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price') }}" placeholder="Contoh: 12000" min="0" required>
                </div>

                <div class="mb-3">
                    <label for="description" class="form-label small fw-bold text-secondary">Deskripsi Menu</label>
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Deskripsikan komposisi atau keunggulan menu..." required>{{ old('description') }}</textarea>
                </div>

                <div class="mb-4">
                    <label for="image" class="form-label small fw-bold text-secondary">Foto Menu (Opsional)</label>
                    <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image" accept="image/*">
                    <div class="form-text text-muted small">Format didukung: JPG, PNG, JPEG, WEBP. Ukuran maksimal 2MB.</div>
                </div>
